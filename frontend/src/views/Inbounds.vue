...
const findInbounsUsers = (i: Inbound): string[] | null => {
  if (!inboundWithUsers.includes(i.type)) return null
  const users = clients.value.filter(c => c.inbounds.includes(i.id)).map(c => c.name)
  return users
}
