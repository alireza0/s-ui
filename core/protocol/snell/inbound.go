...
type Inbound struct {
	...
	// multiService is the same value as service when the inbound is
	// multi-user, kept typed so users can be updated in place.
	multiService multiUserService
	// users is replaced wholesale by UpdateUsers while connections read it.
	users atomic.Pointer[[]option.SnellUser]
}

// multiUserService is implemented by the v5 and v6 multi-user services.
type multiUserService interface {
	UpdateUsers(users []int, userKeys [][]byte) error
}
...
func (h *Inbound) newConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) {
	...
	if users := *h.users.Load(); len(users) > 0 {
